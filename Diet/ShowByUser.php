...
$sql_marosRatio = "SELECT * FROM user_diet_ratio WHERE id='$user_id' ORDER BY date DESC LIMIT 1";
...
while ($row = mysqli_fetch_array($result3))
{
    array_push($response_ratio, 
        array(
            "proteinsratio" =>$row["proteins"],
            "fatsratio"     =>$row["fats"],
            "carbsratio"    =>$row["carbs"]
            )
    );
}
